...
import { GuideCategory, garnishesGuideData } from "./guide-data";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";


type GarnishesGuideProps = {
  children: ReactNode;
  existingGarnishes: string[];
};
...
export function GarnishesGuide({ children, existingGarnishes }: GarnishesGuideProps) {
  const existingNamesNormalized = useMemo(() => new Set(existingGarnishes.map(normalizeString)), [existingGarnishes]);
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-6xl">
        <DialogHeader>
          <DialogTitle>Guide des Garnitures & Recettes Standards</DialogTitle>
          <DialogDescription>
            Votre mémo pour suivre les garnitures à créer. Les éléments cochés existent déjà.
          </DialogDescription>
        </DialogHeader>
        <ScrollArea className="h-[75vh]">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-1 pr-6">
                {garnishesGuideData.map((category) => (
                    <CategoryCard 
                        key={category.title}
                        category={category}
